...
class CheemsCrossAttention(nn.Module):
    ...
    def __init__(self, config: CheemsConfig):
        ...
        # Non-Wo-Attention
        self.text_non_wo_attention = NON_WO_ATTENTION_CLASSES[config._attn_implementation](config, self.intermediate_size)
        self.feature_non_wo_attention = NON_WO_ATTENTION_CLASSES[config._attn_implementation](config, self.intermediate_size)

        # 可以学习的缩放因子 (2个)
        self.text_scale = nn.Parameter(torch.ones(self.intermediate_size))
        self.feature_scale = nn.Parameter(torch.ones(self.intermediate_size))
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Non-Wo-Attention
        text_hidden_states, _, _ = self.text_non_wo_attention(text_hidden_states, attention_mask=text_attention_mask)
        feature_hidden_states, _, _ = self.feature_non_wo_attention(feature_hidden_states, attention_mask=feature_attention_mask)
...
class CheemsCrossTransformer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        text_hidden_states = self.text_decoder(text_hidden_states)
        ...
        feature_hidden_states = self.feature_decoder(feature_hidden_states)
